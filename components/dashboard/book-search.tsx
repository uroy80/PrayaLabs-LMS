...
export function BookSearch() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      await Promise.all([loadCategories(), loadAllBooks(), searchBooks()])
    } catch (error) {
  ...
  const loadCategories = async () => {
    try {
      console.log("🔍 Loading categories from books...")
      const cats = await libraryAPI.getCategories()
      console.log("✅ Categories loaded:", cats)
      setCategories(cats)
    } catch (error) {
      console.warn("❌ Categories loading failed:", error.message)
      // Set minimal fallback categories
      setCategories(["Fiction", "Non-Fiction", "Science", "Technology", "General"])
    }
  }
  ...
  const searchBooksWithField = async (searchField: "title" | "author" | "isbn" | "all" = "all", newPage = 1) => {
    ...
    try {
      ...
      console.error("❌ Books API error:", apiError)
              ...
              <div className="flex gap-2">
                ...
                {(searchQuery || category !== "all") && (
                  <Button
                    variant="outline"
                    onClick={clearSearch}
                    className="h-10 px-4 border-2 border-gray-200 hover:bg-gray-50"
                  >
            ...
            <Card key={book.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {book.category && <p className="text-sm text-gray-600">Category: {book.category}</p>}
      ...
      {totalBooks > 12 && (
        <div className="flex justify-center">
          <div className="flex gap-2">
            ...
            <Button variant="outline" disabled={loading} onClick={() => searchBooksWithField("all", page + 1)}>
